...
import { Sun, Moon, Bell, Shield, Zap, CheckCircle } from 'lucide-react'

// --- COMPONENTE TOGGLE (DISEÑO RESPONSIVE DEFINITIVO) ---
const SettingToggle = ({ icon: Icon, title, desc, isActive, onClick }) => (
  <div 
    onClick={onClick} 
    className="flex flex-col md:flex-row md:items-center justify-between p-6 bg-gray-50 dark:bg-[#1a1a1a] rounded-2xl border border-gray-100 dark:border-gray-800 cursor-pointer group transition-all hover:border-gray-300 dark:hover:border-gray-600 gap-4"
  >
      {/* 1. SECCIÓN SUPERIOR: Icono y Texto */}
      <div className="flex items-start gap-4 w-full"> 
        <div className={`p-3 rounded-full transition-colors duration-300 shrink-0 ${isActive ? 'bg-black dark:bg-white text-white dark:text-black' : 'bg-gray-200 dark:bg-gray-800 text-gray-500 dark:text-gray-400'}`}>
           <Icon size={24} />
        </div>
        <div>
           <h3 className="font-bold uppercase dark:text-white group-hover:text-red-600 transition-colors text-sm md:text-base whitespace-normal">
             {title}
           </h3>
      ...
      {/* 2. SECCIÓN INFERIOR: Switch */}
      <div className="w-full md:w-auto flex justify-end mt-2 md:mt-0">
          <div className={`w-14 h-8 rounded-full p-1 transition-colors duration-300 flex items-center shrink-0 ${isActive ? 'bg-red-600 justify-end' : 'bg-gray-300 dark:bg-gray-700 justify-start'}`}>
...
export default function ConfiguracionPage() {
  ...
  // 1. ESTADO INICIAL DE LAS OTRAS CONFIGURACIONES
  ...
  // 3. FUNCIÓN PARA CAMBIAR AJUSTES GENERALES
  ...
  // 4. FUNCIÓN NUEVA: CAMBIAR TEMA + MOSTRAR TOAST
  const handleThemeChange = () => {
    toggleTheme() // Cambia el color real
    triggerToast() // Muestra el mensajito
  }
  ...
  return (
    <div className="container mx-auto px-6 pt-24 pb-12 min-h-screen">
      ...
      <div className="space-y-8 max-w-3xl">
         ...
         <div>
            ...
            <SettingToggle 
               ...
               isActive={isDarkMode}
               onClick={handleThemeChange} // AHORA USAMOS LA NUEVA FUNCIÓN
            />
         </div>
      ...
      <div className={`fixed bottom-8 right-8 z-50 bg-black dark:bg-white text-white dark:text-black px-6 py-4 rounded-xl shadow-2xl flex items-center gap-3 transition-all duration-300 transform ${showToast ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0 pointer-events-none'}`}>
